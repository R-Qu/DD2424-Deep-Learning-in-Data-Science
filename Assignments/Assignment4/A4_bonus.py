...
DATA_FILENAME = "./../dataset/condensed_2018.json"
...
def read_data():
    ...
    with open(DATA_FILENAME) as f:
        data = json.load(f)
        book_data = "".join(d['text'] for d in data)
...
class Vanilla_RNN:
    ...
    def fit(self, book_data):
        ...
        n = len(book_data)
        nb_seq = math.ceil(float(n-1) / float(self.seq_length))
        smooth_loss = 0
        ite = 0
        losses = []
...
def plot():
    with open("loss.npz.npy") as f:
        loss = list(np.load("loss.npz.npy").reshape(1057, 1))
    print(loss)
